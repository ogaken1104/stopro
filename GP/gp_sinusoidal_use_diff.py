...
class GPSinusoidalUseDifF(GPmodel2D):
    def __init__(
        self,
    ...
    def trainingK_all(self, θ, train_pts):
        ...
        ######### New ##########
        Kuxdiffx = self.setup_kernel_include_difference_prime(Kuxfx)
        Kuxdiffy = self.setup_kernel_include_difference_prime(Kuxfy)
        Kuydiffx = self.setup_kernel_include_difference_prime(Kuyfx)
        Kuydiffy = self.setup_kernel_include_difference_prime(Kuyfy)
        Kdifuxdiffx = self.setup_kernel_difdif(Kuxfx)
        Kdifuxdiffy = self.setup_kernel_difdif(Kuxfy)
        Kdifuydiffx = self.setup_kernel_difdif(Kuyfx)
        Kdifuydiffy = self.setup_kernel_difdif(Kuyfy)
        ...
        Kfxdiffx = self.setup_kernel_include_difference_prime(Kfxfx)
        Kfxdiffy = self.setup_kernel_include_difference_prime(Kfxfy)
        Kfydiffx = self.setup_kernel_include_difference_prime(Kfyfx)
        Kfydiffy = self.setup_kernel_include_difference_prime(Kfyfy)

        Kdiffxdiffx = self.setup_kernel_difdif(Kfxfx)
        Kdiffxdiffy = self.setup_kernel_difdif(Kfxfy)
        Kdiffydiffx = self.setup_kernel_difdif(Kfyfx)
        Kdiffydiffy = self.setup_kernel_difdif(Kfyfy)

        Kdiffxdiv = self.setup_kernel_include_difference(Kfxdiv)
        Kdiffydiv = self.setup_kernel_include_difference(Kfydiv)
        Kdiffxdifp = self.setup_kernel_difdif(Kfxdifp)
        Kdiffydifp = self.setup_kernel_difdif(Kfydifp)
        ...
        Ks = [
            ...
            [Kfxfx, Kfxfy, Kfxdiffx, Kfxdiffy, Kfxdiv, Kfxdifp],
            [Kfyfy, Kfydiffx, Kfydiffy, Kfydiv, Kfydifp],
            [Kdiffxdiffx, Kdiffxdiffy, Kdiffxdiv, Kdiffxdifp],
            [Kdiffydiffy, Kdiffydiv, Kdiffydifp],
            [Kdivdiv, Kdivdifp],
            [Kdifpdifp],
        ]
    ...
    def mixedK_all(self, θ, test_pts, train_pts):
        ...
        if self.infer_governing_eqs:
            ...
            Kfydiffx = self.setup_kernel_include_difference_prime(Kfyfx)
            Kfydiffy = self.setup_kernel_include_difference_prime(Kfyfy)
            Kdivdiffx = self.setup_kernel_include_difference_prime(Kdivdiffx)
            Kdivdiffy = self.setup_kernel_include_difference_prime(Kdivdiffy)
            Ks = [
                [
                    Kfxux,
                    Kfxuy,
