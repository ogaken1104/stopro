...
def plot_val_lattice(p, vname, ax, vlimit, grid):
    ...
    if vname == 'ux':
        ...
        cmap = cmo.cm.delta
    mappable = ax.pcolormesh(grid, grid, p, cmap=cmap,
                             vmin=vlimit[0], vmax=vlimit[1], shading="gouraud")
...
def plot_error_lattice(err, vname, ax, limit, grid):
    ...
    # ax.set_xlabel("x")
    # ax.set_ylabel("y")
    mappable = ax.pcolormesh(grid, grid, err, cmap=mpl.cm.cool, norm=mpl.colors.LogNorm(
        vmin=limit[0], vmax=limit[1]), shading='gouraud')
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.1)
    ax.set_aspect('equal', adjustable='box')
    return mappable, cax
...
def plot_std_lattice(p, vname, ax, limit, grid):
    ...
    # ax.set_xlabel("x")
    # ax.set_ylabel("y")
    mappable = ax.pcolormesh(grid, grid, p, cmap=mpl.cm.cool,
                             vmin=limit[0], vmax=limit[1], shading="gouraud")
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.1)
    ax.set_aspect('equal', adjustable='box')
...
def plot_each_sinusoidal(contents, params, lbls, vnames, infer_p):
    ...
    if 'all' in contents:
        ...
        for vname, r_te, err, f_infer, st, axs, vlimit in zip(vnames['test'], r_test, abs_error, f, std, axes, val_limits):
            ...
            mappable = axs[ax_index].pcolormesh(
                x_grid, y_grid, f_mesh, cmap=cmaps[index], vmin=vlimit[0], vmax=vlimit[1], shading="gouraud")
            ...
            f_mesh = f_for_plot.reshape(y_num, x_num)
            mappable = axs[ax_index].pcolormesh(x_grid, y_grid, f_mesh, cmap=mpl.cm.cool, norm=mpl.colors.LogNorm(
                vmin=error_limit[0], vmax=error_limit[1]), shading="gouraud")
            ...
            f_mesh = f_for_plot.reshape(y_num, x_num)
            mappable = axs[ax_index].pcolormesh(x_grid, y_grid, f_mesh, cmap=mpl.cm.cool, norm=mpl.colors.LogNorm(
                vmin=std_limit[0], vmax=std_limit[1]), shading="gouraud")
    ...
    if 'rel_error' in contents:
        ...
        for vname, r_te, f_te, rel_err, ax in zip(vnames['test'], r_test, f_test, rel_error, axs):
            ax.plot(xs, y_top, color='k')
            ax.plot(xs, y_bottom, color='k')
            ax.set_title(vname)
            dot_size = 25
            ...
            mappable = ax.scatter(r_te_plot[:, 0], r_te_plot[:, 1], c=rel_err, marker='o', s=dot_size,
                                  cmap=mpl.cm.cool, norm=mpl.colors.LogNorm(vmin=0.001, vmax=1.))
            divider = make_axes_locatable(ax)
            cax = divider.append_axes("right", size="5%", pad=0.1)
            fig.colorbar(mappable, cax=cax)
            ax.set_aspect('equal', adjustable='box')
    ...
    if 'gradp' in contents:
        ...
        val_limits = [[-1., 1.], [-1., 1.]]
        for vname, r_te, err, f_infer, st, axs, vlimit in zip(vnames['test'][2:], r_test[2:], abs_error[2:], f[2:], std[2:], axes, val_limits):
        ...
        fig.tight_layout()
        fig.savefig(f'../fig/gradp_all.png',
                    bbox_inches="tight")
        plt.clf()
        plt.close()
...
def plot_each_lattice(contents, plot_param, lbls, vnames):
    ...
    if 'all' in contents:
        ...
        for vname, r_te, err, f_infer, st, axs, vlimit in zip(vnames['test'], r_test, abs_error, f, std, axes, val_limits):
            ...
            shrink = 1
            # 値のプロット
            mappable, cax0 = plot_val_lattice(
                f_infer_mesh, vname, axs[0], vlimit, grid)
            ...
            # 標準偏差のプロット
            mappable, cax2 = plot_std_lattice(
                std_mesh, vname, axs[2], std_limit, grid)
    ...
    if 'ux' in contents:
        ...
        for x_for_ux in x_for_ux_range:
            ...
            fig, ax = plt.subplots()
            ax.plot(r_plot, f_test_plot,
                    color=COLOR['superfine'], label='analytical', linestyle='--')
            ax.plot(r_plot, f_infer_plot,
                    color=COLOR['mid'], label='inference')
            ax.set_title(f'ux at x = {x_for_ux}')
            ax.legend()
            fig.savefig(f'../fig/ux_{x_for_ux}.png',
                        bbox_inches="tight", )
            plt.clf()
            plt.close()
...
def plot_each_gradp(contents, plot_param, lbls, vnames):
    ...
    abs_error = hdf_operator.load_analysis_data(
        ['abs_error'], ['ux', 'uy', 'px', 'py'])[0]
    f, std = hdf_operator.load_infer_data(
        ['f', 'std'], ['ux', 'uy', 'px', 'py'])
    ...
    if 'ux_wall' in contents:
        ...
        x_ans = [0., 1.]
        ux_ans = [1., 1.]
        ax.plot(x_ans, ux_ans,
                color=COLOR['superfine'], label='answer', linestyle='--')
        r_ux = r_test[0]
        index_for_plot = (r_ux[:, 1] == 1.)
        r_ux_for_plot = r_ux[index_for_plot][:, 0]
        ux_for_plot = f_infer[0][index_for_plot]
        std_for_plot = std[0][index_for_plot]
        ax.plot(r_ux_for_plot, ux_for_plot,
                color=COLOR['mid'], label='inference')
        ax.fill_between(r_ux_for_plot, ux_for_plot-2*std_for_plot, ux_for_plot +
                        2*std_for_plot, color=COLOR['dark'], alpha=0.3, label=r'$\pm 2\sigma$')
        fig.savefig(f'../fig/ux_upper_wall.png',
                    bbox_inches="tight")
        plt.clf()
        plt.close()
