...
def plot_output_all(
    ...
    fig, axes = plt.subplots(
        figsize=(5 * 3, 5 * 2), nrows=2, ncols=3, sharex=True, sharey=True
    )
    ...
    for i, axs in enumerate(axes):
        ...
        f[i] = np.array(f[i])
        f_for_plot = sample.change_outside_values_to_zero(r_test[i], f[i])
        f_mesh = f_for_plot.reshape(num_per_side, num_per_side)
        sample.plot_heatmap(
            fig,
            axs[ax_index],
            x_grid,
        ...
        f_for_plot = sample.change_outside_values_to_zero(r_test[i], abs_error[i])
        f_mesh = f_for_plot.reshape(num_per_side, num_per_side)
        sample.plot_heatmap(
            fig,
            axs[ax_index],
        ...
        f_for_plot = sample.change_outside_values_to_zero(r_test[i], std[i])
        f_mesh = f_for_plot.reshape(num_per_side, num_per_side)
        sample.plot_heatmap(
            fig,
            axs[ax_index],
