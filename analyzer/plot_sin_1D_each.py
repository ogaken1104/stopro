...
def plot_each_sin1D(contents, plot_param, lbls, vnames):
    ...
    if 'all' in contents:
        ...
        ax.plot(r_test[0], f[0], label='infer',
                linestyle='-', color=COLOR['dark'])
        # plot test data
        ax.plot(r_test[0], f_test[0], label='answer',
                linestyle='--', color='k')
        ax.scatter(r_train[0], np.zeros(len(r_train[0])),
                   label=r'$y$ training', marker='o', color='k', facecolor='None')
        ax.scatter(r_train[1], np.zeros(len(r_train[1])),
                   label=r'$\Delta y$ train', marker='x', color='k')
        ...
        if len(r_train) == 3:
            ax.scatter(r_train[2], f_train[2], label='pbc',
                       marker='+', color='k', s=140)
        ax.fill_between(r_test[0], f[0]-2*std[0], f[0] +
                        2*std[0], color=COLOR['dark'], alpha=0.3, label=r'$\pm 2\sigma$')
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_ylim(-1.1, 1.1)
        ax.legend()
        fig.savefig(f'../fig/plot.png', bbox_inches="tight", )
        plt.clf()
        plt.close()
    ...
    if 'abs_error' in contents:
        fig, ax = plt.subplots()
        plt.yscale('log')
        ax.plot(r_test[0], abs_error[0], color='k',
                )
