...
class Cylinder(Drag):
    ...
    def generate_u(self, u_num_surface, u_num_wall, use_inner=False):
        ...
        r_uy_wall = self.make_r_mesh(0., self.L, 0., self.H, u_num_wall, 2)
    ...
    def generate_div(self, div_num, div_pad, num_inner, dr, div=0.):
        r_div = self.make_r_mesh_mixed(
            num_inner, div_num, dr, div_pad)
    ...
    def plot_train(self, save=False, path=None, show=False):
        ...
        clrs = [self.COLOR['dark_highlight'], self.COLOR['dark_highlight'], self.COLOR['superfine'], self.COLOR['superfine'],
                'darkblue', 'darkblue', 'darkblue', self.COLOR['superfine'], self.COLOR['light_highlight']]
        ...
        for i, ax in enumerate(axes):
            if i == 0 or i == 1:
                ...
                ax.plot(self.r[i][:, 0], self.r[i][:, 1],
                        ls='None', marker='o', color=clrs[i], ms=ms2, label='b.c.')
                index = i + 2
