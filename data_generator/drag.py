...
class Drag(StokesDataGenerator):
    ...
    def make_r_surface(self, num, use_inner=False):
        ...
        if not use_inner:
            ...
            radius_range = np.linspace(0.013, self.particle_radius, 5)
    ...
    def generate_test(self, test_num=None):
        try:
            with open(
                "/work/jh210017a/q24015/template_data/0308_drag_test_484.pickle", "rb"
            ) as file:
    ...
    def generate_f(self, f_num, f_pad, num_inner, force=0.0):
    ...
    def generate_div(self, div_num, div_pad, num_inner, div=0.0):
        r_div = self.make_r_mesh_mixed(num_inner, div_num, self.dr, div_pad)
    ...
    def generate_dif(self, dif_num):
        ...
        r_dif_y = self.make_r_mesh(
            0 + self.slide, self.L - self.slide, 0.0, self.H, dif_num, 1
        )
        ...
        for i, ax in enumerate(axes):
            if i == 0 or i == 1:
                ...
                ax.plot(
                    ...
                    ls="None",
                    marker="o",
                    color=self.COLOR["light_highlight"],
                    ms=ms,
                    label="$y$ p.b.c.",
                )
                # if i == 1:
    ...
    def plot_test(self, save=False, path=None, val_limits=None):
        ...
        for i, ax in enumerate(axs):
            ...
            self.plot_heatmap(
                fig, ax, x_grid, y_grid, f_mesh, shading="gouraud", cmap=cmo.cm.balance
            )
        fig.tight_layout()
        if save:
            dir_path = f"{path}/fig"
