...
class Drag(StokesDataGenerator):
    ...
    def get_index_in_domain(self, r, radius_min=None):
        ...
        if not radius_min:
            index_in_domain = np.where(
                rx**2+ry**2 > self.particle_radius**2)[0]
        else:
    ...
    def make_r_mesh_mixed(self, num_inner, num_outer, dr, pad):
        ...
        r_rectangular = self.make_r_mesh_rectangular(
            num_outer, num_outer, pad, radius_min)
        r = np.concatenate([r_circular, r_rectangular])
        return r

    def generate_training_data(self, u_num=None, f_num=None, f_pad=None, div_num=None, div_pad=None, difu_num=None, difp_num=None, num_inner=5):
        self.r = []
        self.f = []
        self.generate_u(u_num)
        self.generate_difu(difu_num)
        self.generate_f(f_num, f_pad, num_inner)
        self.generate_div(div_num, div_pad, num_inner)
        self.generate_difp(difp_num)
        return self.r, self.f

    def generate_test(self, test_num=None):
        try:
            with open('/work/jh210017a/q24015/template_data/0308_drag_test_484.pickle', 'rb') as file:
    ...
    def generate_f(self, f_num, f_pad, num_inner, force=0.):
        r_fx = self.make_r_mesh_mixed(num_inner, f_num, self.dr, f_pad)
        r_fy = self.make_r_mesh_mixed(num_inner, f_num, self.dr, f_pad)
    ...
    def generate_div(self, div_num, div_pad, num_inner, div=0.):
        r_div = self.make_r_mesh_mixed(
            num_inner, div_num, self.dr, div_pad)
    ...
    def generate_dif(self, dif_num):
        r_dif_x = self.make_r_mesh(
            0, self.L, self.slide, self.H-self.slide, 1, dif_num)
        r_dif_y = self.make_r_mesh(
            0+self.slide, self.L-self.slide, 0., self.H, dif_num, 1)
