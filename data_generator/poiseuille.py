...
class Poiseuille(StokesDataGenerator):
    def __init__(self, Re=None, L=None, delta_p=None, pin=None, slide=0.03, random_arrange=True, use_gradp_training=False, infer_gradp=False, infer_ux_wall=False, use_only_bottom_u=False, use_only_inlet_gradp=False, cut_last_x=False, use_difp=False, use_difu=False):
        ...
        if self.delta_p is not None:
            self.f_ux = lambda r: -0.5*self.delta_p*self.Re*r[:, 1]*(1-r[:, 1])
...
#         self.f_p=lambda r:self.delta_p/self.L*r[0]+self.pin
        ...
        if self.infer_ux_wall:
            ...
            if self.random_arrange:
                r_ux = self.make_r_mesh_random(
                    0.+self.slide, x_last-self.slide, 0., 1., u_num-2, num_along_y, self.slide, 'x')
                r_ux = np.concatenate(
                    [r_ux, np.array([[0., 0.], [0., 1.], [x_last, 0.], [x_last, 1.]])])
                r_uy = self.make_r_mesh_random(
                    0.+self.slide, x_last-self.slide, 0., 1., u_num-2, num_along_y, self.slide, 'x')
                r_uy = np.concatenate(
                    [r_uy, np.array([[0., 0.], [0., 1.], [x_last, 0.], [x_last, 1.]])])
            else:
                r_ux = self.make_r_mesh(0., x_last, 0., 1., u_num, num_along_y)
            r_uy = self.make_r_mesh(0., x_last, 0., 1., u_num, num_along_y)
            # without_p test
    ...
    def generate_test(self, test_num, ux_test=False):
        if not ux_test:
            r = self.make_r_mesh(0., self.L, 0., 1., test_num, test_num)
            ux_test = self.f_ux(r)
            ...
            r_ux = self.make_r_mesh(0., self.L, 0., 1., 5, 100)
    ...
    def generate_p(self, p_num):
        ...
        if self.random_arrange:
            r_p = self.make_r_mesh_random(
                0., x_last, 0.+self.slide, 1.-self.slide, num_along_y, p_num-2, self.slide, 'y')
            r_p = np.concatenate(
                [r_p, np.array([[x_last, 0.], [x_last, 1.], [0., 0.], [0., 1.]])])
        elif not self.use_difp:
            r_p = self.make_r_mesh(0., x_last, 0., 1., num_along_y, p_num)
            # without_p test
            # r_p = np.array([[0., 0.]])
        elif self.use_difp:
            r_p = np.array([[0., 0.5]])
    ...
    def generate_gradp(self, p_num):
        ...
        if self.random_arrange:
            r_p = self.make_r_mesh_random(
                0., 1., 0.+self.slide, 1.-self.slide, num_along_x, p_num-2, self.slide, 'y')
            r_p = np.concatenate(
                [r_p, np.array([[1., 0.], [1., 1.], [0., 0.], [0., 1.]])])
        else:
            r_p = self.make_r_mesh(0., 1., 0.+self.slide,
                                   1.-self.slide, num_along_x, p_num)
            # without_p test
            # r_p = np.array([[0., 0.]])
    ...
    #         fx=np.full(f_num,force)
            ...
            r_fx = self.make_r_mesh(
                x_start, x_end, y_start, y_end, f_num, f_num)
            r_fy = self.make_r_mesh(
                x_start, x_end, y_start, y_end, f_num, f_num)
    ...
    def generate_div(self, div_num, div_pad, divu=0.):
        ...
        if self.random_arrange:
            ...
            r_div = self.make_r_mesh(
                x_start, x_end, y_start, y_end, div_num, div_num)
        div = np.full(len(r_div), divu)
        self.r += [r_div]
        self.f += [div]

    def plot_test(self, val_limits, save=False, path=None):
        fig, axs = plt.subplots(
            figsize=(5*3, 3), ncols=3, sharey=True)
        lbls = ['ux', 'uy', 'p']
        ...
        for i, ax in enumerate(axs):
            ...
            mappable = ax.scatter(self.r_test[i][:, 0], self.r_test[i][:, 1], c=self.f_test[i],
                                  marker='o', cmap=cmo.cm.dense, vmin=val_limits[i][0], vmax=val_limits[i][1])
            ax.set_title(lbls[i])
...
#         return [r_ux]*3
    ...
    def plot_train(self, save=False, path=None):
        if self.use_gradp_training and self.use_difu:
            ...
            clrs = [self.COLOR['mid'], self.COLOR['mid'], self.COLOR['superfine'], self.COLOR['superfine'],
                    'green', 'green', self.COLOR['light']]
            lbls = ['$u_x$', '$u_y$', '$\partial_x p$',
                    '$\partial_y p$', 'fx', 'fy', 'div']
        ...
        for i, ax in enumerate(axes):
            ...
            if lbls[i] == 'p' and self.use_difp == True:
                ax.plot(self.r[-1][:, 0], self.r[-1][:, 1], ls='None', marker='o',
                        color=self.COLOR['dark'], label='Delta_p')
    ...
    def generate_difp(self, difp_num, difp_pad, difp_loc):
        if difp_loc == 'all_inside':
            ...
            r_difp = self.make_r_mesh(
                0., 0., 0.+self.slide, 1.-self.slide, 1, difp_num)
    ...
    def generate_difu(self, difu_num):
        ...
        r_difu = self.make_r_mesh(
            0., 0., 0.+self.slide, 1.-self.slide, 1, difu_num)
