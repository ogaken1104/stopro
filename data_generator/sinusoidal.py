...
class Sinusoidal(StokesDataGenerator):
    def __init__(self, oscillation_amplitude=None, channel_length=None, channel_width=None, slide=0.03, random_arrange=False, use_gradp_training=False, infer_gradp=False, infer_ux_wall=False, use_only_bottom_u=False, use_only_inlet_gradp=False, cut_last_x=False, use_difp=False, use_difu=False, use_inlet_outlet_u=False, x_start=0., use_random_u=False, delta_p=-30, use_force_as_constant_pressure=False, use_1d_u=False):
        ...
        self.gradpy = 0.  # only satisfied in inlet and outlet
    ...
    def calc_u2(self, eta, xi):
        ...
        first_2 = 2 * self.a + jnp.sin(2 * jnp.pi * eta) + 6 * \
            self.a * jnp.cos(2 * jnp.pi * eta)**2
    ...
    def calc_v2(self, eta, xi):
        ...
        first_2_2 = 2 * self.a + jnp.sin(2 * jnp.pi * eta) + \
            6 * self.a * jnp.cos(2 * jnp.pi * eta)**2
        first_2_3 = 5 * xi ** 5 - 6 * xi ** 3 + xi
        second_1 = 3 * self.a * self.Q2 / 2 / \
            (1 + 2 * self.a * jnp.sin(2 * jnp.pi * eta))
        second_2 = -xi ** 3 + xi
        first = first_coef * (first_1_1 * first_1_2 +
                              first_2_1 * first_2_2 * first_2_3)
    ...
    def calc_Q4(self):
        ...
        integral, _abs_error = integrate.quad(integrand, 0., 1.)
#         print(f'integral={integral}')
    ...
    def calc_Q(self):
        return self.calc_Q0() + self.epsilon**2 * self.calc_Q2() + self.epsilon**4 * self.calc_Q4()
    ...
    def calc_psi_4(self, x, y):
        ...
        term3 = - Q0 / 5600 * (first + second + third +
                               fourth + fifth) * xi * (1 - xi**2)**2
    ...
    def change_outside_values_to_zero(self, r, f):
        ...
        is_outside_the_domain = np.where((r[:, 1] > self.calc_y_top(r[:, 0])) | (
            r[:, 1] < self.calc_y_bottom(r[:, 0])))
        f[is_outside_the_domain] = 0.
        return f
    ...
    def generate_u(self, u_num, u_b=0.):
        ...
        if self.use_random_u:
            ...
            index2_x = 21
            xs_for_ux = np.array(
                [xs_unique[index_x], xs_unique[-index_x]])
            ...
            if self.use_inlet_outlet_u:
                r_u_inlet_outlet = self.make_r_mesh(
                    self.x_start, self.x_end, -self.w/2+self.slide, self.w/2-self.slide, 2, u_num_y)
                # eta_u_inlet_outlet = self.x_to_eta(r_u_inlet_outlet[:, 0])
                # xi_u_inlet_outlet = self.y_to_xi(
                #     eta_u_inlet_outlet, r_u_inlet_outlet[:, 1])
                ux_inlet_outlet, uy_inlet_outlet = self.calc_u_v_4(
                    r_u_inlet_outlet)
                # concatenate
...
#             xi_u = self.y_to_xi(eta_u, r[:, 1])
            ...
            is_outside_the_domain = np.where((r[:, 1] > self.calc_y_top(r[:, 0])) | (
                r[:, 1] < self.calc_y_bottom(r[:, 0])))
            ux_test[is_outside_the_domain] = 0.
            uy_test[is_outside_the_domain] = 0.
            if self.use_gradp_training:
                if not self.infer_gradp:
                    r_p = np.array([[self.x_start, 0.]])
                    p_test = np.zeros(len(r_p))
                elif self.infer_gradp:
                    r_p = self.make_r_mesh(
                        self.x_start, self.x_end, -self.w/2+self.slide, self.w/2-self.slide, 2, int(test_num/2))
                    print(r_p, test_num)
            ...
            r_ux = self.make_r_mesh(self.x_start, self.x_end, 0., 1., 5, 100)
            ux_test = self.f_ux(r_ux)
            r_uy = np.array([[self.x_start, 0.]])
    ...
    def generate_f(self, f_num, f_pad, force=0.):
        ...
        f_num_x, f_num_y = self.num_to_num_x_y(f_num)
    ...
    def generate_difp(self, difp_num, difp_pad, difp_loc):
        ...
        if difp_loc == 'all_inside':
            r_difp = self.make_r_mesh_sinusoidal(
                x_start, x_end, y_start, y_end, int(difp_num_x/4), difp_num_y, difp_pad)
        elif difp_loc == 'inlet_outlet':
            # use difp on inlet and outlet
            r_difp = self.make_r_mesh_sinusoidal(
                0., 0., -self.w/2+self.slide, self.w/2-self.slide, 1, difp_num, self.slide)
        elif difp_loc == '1pair':
            r_difp = np.array([[0., 0.]])
    ...
    def generate_gradp(self, p_num):
        ...
        r_gradpx = self.make_r_mesh_sinusoidal(
            self.x_start, self.x_end, -self.w/2+self.slide, self.w/2-self.slide, num_along_x, p_num, self.slide)
    ...
    def generate_training_data(self, u_num, p_num, f_num, f_pad, div_num, div_pad, difp_num=None, difp_pad=None, difp_loc='all_inside', difu_num=None):
        self.r, self.f = super().generate_training_data(
            u_num, p_num, f_num, f_pad, div_num, div_pad, difp_num, difp_pad, difp_loc, difu_num)
        # if self.use_difp:
        #     self.generate_difp(difp_num, difp_pad, difp_loc)
        return self.r, self.f

    def make_r_mesh_sinusoidal(self, x_start, x_end, y_start, y_end, num_x, num_y, pad,  periodic_test=False):
    ...
    def plot_test(self, val_limits=[[0., 1.], [0., 1.], [0., 1]], save=False, path=None):
        fig, axs = plt.subplots(
            figsize=(5*2, 3), ncols=2, sharex=True, sharey=True)
        ...
        for i, ax in enumerate(axes):
            ...
            if i == 0 or i == 1:
                ...
                mappable = ax.pcolormesh(
                    x_grid, y_grid, f_mesh, cmap=cmaps[i], vmin=val_limits[i][0], vmax=val_limits[i][1], shading="gouraud")
            elif i == 2:
                mappable = ax.scatter(self.r_test[i][:, 0], self.r_test[i][:, 1],
                                      c=self.f_test[i], marker='o', s=25., cmap=cmaps[i], vmin=val_limits[i][0], vmax=val_limits[i][1])
    ...
    def plot_train(self, save=False, path=None):
        ...
        COLOR = {i[0]: i[1] for i in zip(['light', 'light_highlight', 'mid',  'mid_highlight',
        ...
        if self.use_gradp_training and self.use_difu:
            ...
            clrs = [self.COLOR['mid'], self.COLOR['mid'], self.COLOR['dark_highlight'], self.COLOR['dark_highlight'], self.COLOR['superfine'], self.COLOR['superfine'],
                    'green', 'green', self.COLOR['light']]
            lbls = ['$u_x$', '$u_y$', '$u_x^{out}-u_x^{in}$', '$u_y^{out}-u_y^{in}$',
                    '$\partial_x p$', '$\partial_y p$', '$f_x$', '$f_y$', r'$\nabla \cdot u$']
            axes = axs.reshape(-1)
        elif self.use_difu and not self.use_difp:
            fig, axs = plt.subplots(
                figsize=(3*3, 3*3), nrows=3, ncols=3, sharex=True, sharey=True)
            clrs = [self.COLOR['mid'], self.COLOR['mid'], self.COLOR['dark_highlight'], self.COLOR['dark_highlight'], self.COLOR['superfine'],
                    'green', 'green']
            lbls = ['$u_x$', '$u_y$', '$u_x^{out}-u_x^{in}$', '$u_y^{out}-u_y^{in}$',
                    '$f_x$', '$f_y$', r'$\nabla \cdot u$']
            ...
            fig, axs = plt.subplots(
                figsize=(3*3, 3*3), nrows=3, ncols=3, sharex=True, sharey=True)
            clrs = [self.COLOR['mid'], self.COLOR['mid'], self.COLOR['dark_highlight'],
                    self.COLOR['dark_highlight'], 'green', 'green', self.COLOR['light'], self.COLOR['dark']]
            lbls = ['$u_x$', '$u_y$', '$u_x^{out}-u_x^{in}$', '$u_y^{out}-u_y^{in}$',
                    '$f_x$', '$f_y$', r'$\nabla \cdot u$', '$p^{out}-p^{in}$']
            fig.delaxes(axs.reshape(-1)[-1])
            ...
            fig.delaxes(axs.reshape(-1)[-1])
            clrs = [self.COLOR['mid'], self.COLOR['mid'], self.COLOR['superfine'], self.COLOR['superfine'],
                    'green', 'green', self.COLOR['light']]
        ...
        for i, ax in enumerate(axes):
            ...
            ax.plot(self.r[i][:, 0], self.r[i][:, 1],
                    ls='None', marker='o', markersize=ms, color=clrs[i])
            ax.set_title(lbls[i])
            ax.set_aspect('equal', adjustable='box')
        fig.tight_layout()
        if save:
    ...
    def generate_difu(self, difu_num, difu_loc='inlet_outlet'):
        ...
        if difu_loc == 'inlet_outlet':
            ...
            r_difu = self.make_r_mesh_sinusoidal(
                self.x_start, self.x_end, -self.w/2+self.slide, self.w/2-self.slide, 1, difu_num, self.slide)
        elif difu_loc == 'all_inside':
            difu_num_x, difu_num_y = self.num_to_num_x_y(difu_num)
            r_difu = self.make_r_mesh_sinusoidal(
                self.x_start-self.L/2, self.x_end-self.L/2, -self.w/2+self.slide, self.w/2-self.slide, difu_num_x, difu_num_y, self.slide)
        difu = np.full(len(r_difu), 0.)
