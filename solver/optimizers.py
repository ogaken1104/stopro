...
def optimize_by_adam(f, df, hf, init, optimize_param, *args):
    ...
    def step(t, opt_state, optimizer, theta):
        value, grads = value_and_grad(func)(
            theta, *args)
    ...
    if maxiter_first:
        if method_first == 'Nelder-Mead' or method_first == 'L-BFGS-B':
            ...
            res.append(optimize.minimize(
                func, res[-1]['x'], args=args, method=method_first, jac=dfunc, options=opt))
...
def optimize_by_adam_avoid_divergence(f, df, hf, init, optimize_param, *args):
    ...
    if maxiter_GD:
        theta, loss = doopt(
            res[-1]['x'], opt_init, get_params, maxiter_GD, loss, theta, norm_of_grads_list)
