...
def test_sin_1D_direct_main():
    ...
    for index_theta in range(len(init)):
        theta_i = init[index_theta]
        theta_other = jnp.delete(init, index_theta)
        # print("\nK")
        # print(
        #     gp_model.calc_K_given_theta_i(
        #         theta_i, theta_other, index_theta, r_train, params_model["epsilon"]
        #     )
        # )

        calc_dKdtheta_i = jax.jacfwd(gp_model.calc_K_given_theta_i, argnums=0)
        print(f"\ndKdtheta_{index_theta}")
        print(
            calc_dKdtheta_i(
                theta_i, theta_other, index_theta, r_train, params_model["epsilon"]
            )
        )
        # dKss = gp_model.setup_dKss_theta_i(index_theta)
        # print("\nK")
        # print(gp_model.trainingKs[0][0](r_train[0], r_train[0], init))
        # print("\ndKdtheta new")
        # print(dKss[0][0](r_train[0], r_train[0], init))
        # print(dKss[1][0](r_train[1], r_train[1], init))

        print("\ndKdtheta new")
        θ, noise = gp_model.split_hyp_and_noise(init)
        dKss_i = gp_model.setup_dKss_theta_i(index_theta)
        dKdtheta_i = gp_model.calculate_K_training(r_train, dKss_i, θ)
        print(dKdtheta_i)

    print(gp_model.d_logposterior(init, *args_predict[2:]))
